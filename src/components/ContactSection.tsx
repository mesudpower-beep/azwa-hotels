      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }} className="space-y-8">
            ...
            <a href="https://wa.me/251998900160" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 border border-primary/40 text-primary px-6 py-3 text-sm tracking-[0.15em] uppercase font-body hover:bg-primary/10 transition-all">
          ...
          <motion.form initial={{ opacity: 0, x: 30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} onSubmit={handleSubmit} className="glass-card p-8 space-y-6">
            <div>
              ...
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} maxLength={100} className="w-full border border-border bg-background/50 px-4 py-3 text-sm font-body text-foreground outline-none focus:border-primary/50 transition-colors" />
            </div>
            <div>
              ...
              <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} maxLength={255} className="w-full border border-border bg-background/50 px-4 py-3 text-sm font-body text-foreground outline-none focus:border-primary/50 transition-colors" />
            </div>
            <div>
              <label className="block text-xs tracking-[0.15em] uppercase text-muted-foreground font-body mb-3">{t("contact.message")}</label>
              <textarea rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} maxLength={1000} className="w-full border border-border bg-background/50 px-4 py-3 text-sm font-body text-foreground outline-none focus:border-primary/50 transition-colors resize-none" />
            </div>
            <button type="submit" className="w-full gold-gradient text-primary-foreground px-8 py-4 text-sm tracking-[0.2em] uppercase font-body font-medium hover:opacity-90 transition-opacity flex items-center justify-center gap-3">
